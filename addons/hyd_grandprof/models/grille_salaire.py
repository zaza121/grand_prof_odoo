...
class GrilleSalaire(models.Model):
    ...
    _name = 'hyd_salary_grid.grille_salaire'
    ...
    categ_id = fields.Many2one(
        comodel_name='hyd_salary_grid.categorie_salariale',
        string="Categorie salariale")

    ech_id = fields.Many2one(
        comodel_name='hyd_salary_grid.echelon_salariale',
        string="Echellon")
    ...
    def update_masse(self):
        for record in self:
            record.masse = sum(record.contracts.filtered(
                lambda x: x.state != 'close').mapped('wage'))
